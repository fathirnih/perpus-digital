                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between">
                        <div>
                            <h1 class="text-3xl md:text-4xl font-bold text-gray-800 mb-2">
                                <span class="bg-gradient-to-r from-yellow-600 to-amber-600 bg-clip-text text-transparent">Edit Buku</span>
                            </h1>
                            <p class="text-gray-600 text-lg">
                                <i class="fas fa-edit text-yellow-500 mr-2"></i>
                                Perbarui data buku: <strong>"{{ $buku->judul }}"</strong>
                            </p>
                        </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200">
                <div class="px-6 py-4 bg-gradient-to-r from-yellow-50 to-amber-50 border-b border-amber-100">
                    <div class="flex items-center">
                        ...
                        <div>
                            <h2 class="text-lg font-semibold text-gray-800">Form Edit Buku</h2>
                            <p class="text-sm text-gray-600">Perbarui data buku di bawah ini</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 md:p-8">
                    ...
                    <form action="{{ route('admin.buku.update', $buku->id) }}" method="POST" class="space-y-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div class="mt-2">
                                    <div class="flex items-center">
                                        <div class="w-full bg-gray-200 rounded-full h-2">
                                            @php
                                                $percentage = min(100, ($buku->jumlah / 50) * 100);
                                                $color = $percentage > 70 ? 'bg-green-500' : ($percentage > 30 ? 'bg-yellow-500' : 'bg-red-500');
                                            @endphp
                                            <div id="stok-progress" class="h-2 rounded-full {{ $color }}" style="width: {{ $percentage }}%"></div>
                                        </div>
                                        <span id="stok-status" class="ml-2 text-xs text-gray-500">{{ $buku->jumlah }} eks</span>
                                    </div>
                                </div>
                                @error('jumlah')
                                    <p class="text-red-600 text-sm mt-2 flex items-center">
                            ...
                            <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-100">
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-3">
                                        <h4 class="text-sm font-medium text-gray-800">Informasi Buku</h4>
                                        <div class="mt-1 space-y-1 text-xs text-gray-600">
                                            <div class="flex items-center">
                                                <i class="fas fa-calendar-alt mr-2 text-gray-400"></i>
                                                Dibuat: {{ $buku->created_at->translatedFormat('d F Y') }}
                                            </div>
                                            <div class="flex items-center">
                                                <i class="fas fa-history mr-2 text-gray-400"></i>
                                                Terakhir diupdate: {{ $buku->updated_at->diffForHumans() }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="pt-6 border-t border-gray-200">
                            <div class="flex flex-col sm:flex-row gap-4">
                                <button type="submit" 
                                        class="inline-flex items-center justify-center px-8 py-4 bg-gradient-to-r from-yellow-500 to-amber-500 text-white font-medium rounded-xl hover:from-yellow-600 hover:to-amber-600 hover:shadow-xl transition-all duration-200 group flex-1">
                                ...
                                <button type="button" 
                                        ...
                                        class="inline-flex items-center justify-center px-8 py-4 bg-gradient-to-r from-red-500 to-pink-500 text-white font-medium rounded-xl hover:from-red-600 hover:to-pink-600 hover:shadow-xl transition-all duration-200 group flex-1">
                    ...
                    <!-- Delete Form (Hidden) -->
                    <form id="delete-form" action="{{ route('admin.buku.destroy', $buku->id) }}" method="POST" class="hidden">
                        @csrf
                        @method('DELETE')
                    </form>
                </div>
            ...
            <div class="mt-8 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl shadow-md p-6 border border-blue-100">
                <div class="flex items-start">
                    ...
                    <div class="ml-4">
                        <h3 class="text-lg font-medium text-gray-900">Tips Mengedit Buku</h3>
                        <div class="mt-2 text-gray-600">
                            <ul class="list-disc pl-5 space-y-1">
                                <li>Perbarui data dengan informasi yang akurat</li>
                                <li>Pastikan jumlah buku sesuai dengan stok fisik</li>
                                <li>Periksa kembali tahun terbit untuk keakuratan data</li>
                                <li>Pilih kategori yang paling sesuai dengan konten buku</li>
                                <li>Data yang sudah diupdate tidak dapat dikembalikan seperti semula</li>
                            </ul>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (jumlahInput && stokProgress && stokStatus) {
        ...
        updateProgressBar(jumlahInput.value || 0);
        
        jumlahInput.addEventListener('input', function() {
            updateProgressBar(this.value || 0);
        });

        function updateProgressBar(value) {
            const numericValue = parseInt(value) || 0;
            let percentage = Math.min(100, (numericValue / 50) * 100);
            ...
            stokStatus.textContent = numericValue + ' eks';
        }
    }
